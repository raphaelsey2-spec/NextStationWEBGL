...
function initBuffers(gl) {
  ...
  return {
    position: positionBuffer[0],
	indices: positionBuffer[1],
  };
}

function triangleSymbole (x, y, h, k) {
	let positions = [];
	let indices = [];
	...
	positions.push(0.35 + x);
	positions.push(0.25 + y);
	positions.push(h);
	positions.push(0);
	...
	return [positions, indices, k];
}

function carreSymbole (x, y, h, k) {
	let positions = [];
	let indices = [];
	...
	positions.push(0.25 + x);
	positions.push(0.25 + y);
	positions.push(h);
	positions.push(0);

	positions.push(0.25 + x);
	positions.push(0.75 + y);
	positions.push(h);
	positions.push(0);
...
function pantagonSymbole (x, y, h, k) {
	let positions = [];
	let indices = [];
	...
	positions.push(0.85 + x);
	positions.push(0 + y);
	positions.push(h);
	positions.push(0);

	positions.push(0.70 + x);
	positions.push(0.25 + y);
	positions.push(h);
	positions.push(0);

	positions.push(0.30 + x);
	positions.push(0.25 + y);
	positions.push(h);
	positions.push(0);

	positions.push(0.15 + x);
	positions.push(0 + y);
	positions.push(h);
	positions.push(0);
...
function circleSymbole (x, y, h, k, r) {
	let positions = [];
	let indices = [];
	...
	for (let i = 0; i <= r; i++) {
	   const angle= (2 * Math.PI * i) / (r / 2);
	   const x1 = x + radius * Math.cos(angle) + radius;
	   const y1 = y + radius * Math.sin(angle) + radius;
	   const x2 = x + radius2 + radius2 * Math.cos(angle) + radius2;
	   const y2 = y + radius2 + radius2 * Math.sin(angle) + radius2;
	   ...
	   indices.push(i + k, i + 2 + k, i + 3 + k);
	   indices.push(i + k, i + 1 + k, i + 3 + k);
	}

	k += positions.length/4;

	return [positions, indices, k];
}

function createStation (x, y, height, k, resolution, symbole, id) {
	const radius = 1;
	let indices = [];
	let positions = [x + radius, y + radius, height, id];

	for (let i = 1; i < resolution + 1; i++) {
	   const angle= (2 * Math.PI * i) / (resolution / 2);
	   const x1 = x + radius * Math.cos(angle) + radius;
	   const y1 = y + radius * Math.sin(angle) + radius;
	   ...
	   positions.push(x1);
	   positions.push(y1);
	   positions.push(height - 10);
	   positions.push(id);

	   indices.push(i + k, i + 2 + k, k);
	   indices.push(i + k, i + 1 + k, i + 3 + k);
	   indices.push(i + k, i + 2 + k, i + 3 + k);
	}
	...
	let tmp_tri = [];
	if (symbole == 0) {
		tmp_tri = triangleSymbole(x + radius, y + radius, height + 0.1, k);
	} else if (symbole == 1) {
		tmp_tri = carreSymbole(x + radius, y + radius, height + 0.1, k);
	} else if (symbole == 2) {
		tmp_tri = pantagonSymbole(x + radius, y + radius, height + 0.1, k);
	} else if (symbole == 3) {
		tmp_tri = circleSymbole(x + radius, y + radius, height + 0.1, k, resolution);
	}
...
function initPositionBuffer(gl) {
  ...
  for (let i = 0; i < mapSize; i++) {
	  for (let l = 0; l < mapSize; l++) {
		  ...
		  if (l > 0) {
			  indices.push(0 + ind);
			  indices.push(2 + ind - 4);
			  indices.push(3 + ind - 4);
			  indices.push(0 + ind);
			  indices.push(3 + ind - 4);
			  indices.push(1 + ind);
		  }
		  ...
		  if (i > 0) {
			  indices.push(2 + ind);
			  indices.push(1 + (ind - (4 * l + (mapSize - l) * 4)));
			  indices.push(3 + (ind - (4 * l + (mapSize - l) * 4)));
			  indices.push(0 + ind);
			  indices.push(1 + (ind - (4 * l + (mapSize - l) * 4)));
			  indices.push(2 + ind);
		  }
  ...
  for (let i = 0; i<50; i++) {
	  let tmp_tri = createStation(Math.random()*100%mapSize, Math.random()*100%mapSize, 5, ind, 50, Math.round(Math.random()*100%3), 4);
	  ind = tmp_tri[2];
	  tmp_tri[0].forEach((x)=>{
		  positions.push(x);
	  });
...
function drawScene(gl, programInfo, buffers) {
  ...
  gl.uniform1f(programInfo.uniformLocations.time, (Date.now() - start)/1000);
...
function startWebGL() {
  ...
  const vsSource = `
	...
	varying mediump float color_id;

    void main(void) {
	  color_id = aVertexPosition.w;
      gl_Position = uProjectionMatrix * uModelViewMatrix * vec4(aVertexPosition.xyz, 1.0);
	  depth = aVertexPosition.z;
    }
  `;
  ...
  const fsSource = `
	...
	//highp float simcos(in mediump vec3 a, in mediump vec3 b) {
	//	highp float value = (     (a.x * b.x + a.y * b.y + a.z * b.z) /(
	//						  sqrt(pow(b.x,2.0)+ pow(b.y,2.0) + pow(b.z,2.0)) *
	//						  sqrt(pow(a.x,2.0)+ pow(a.y,2.0) + pow(a.z,2.0))
	//						 ));
	//	if (value <= 0.0)
	//		value = 0.0;
	//	return value;
    ...
    void main(void) {
		//highp float intensity = simcos(gl_FragCoord.xyz, vec3(1000.0));
		if (color_id == 4.0) {
			gl_FragColor = vec4(((depth/5.0)) * 0.0,
								((depth/5.0)) * 1.0,
								((depth/5.0)) * 0.0,
								1.0);
			...
			gl_FragColor = vec4((1.0 - (depth/5.0)),
								(1.0 - (depth/5.0)),
								(1.0 - (depth/5.0)),
								1.0);
		}
...
setInterval(()=>{
	if ( !isMoving ) {
		let tmpsX = Math.sign(inertia[0]);
		let tmpsY = Math.sign(inertia[1]);

		inertia[0] = Math.abs(inertia[0]);
		inertia[1] = Math.abs(inertia[1]);

		inertia[0] = inertia[0] - 0.01;
		inertia[1] = inertia[1] - 0.01;

		if ( inertia[0] <= 0.0 ) { inertia[0] = 0.0; }
		if ( inertia[1] <= 0.0 ) { inertia[1] = 0.0; }

		inertia[0] = inertia[0] * tmpsX;
		inertia[1] = inertia[1] * tmpsY;
	}
	posX += inertia[0];
	posY += inertia[1];
...
addEventListener("mousemove", (event) => {
	if (isMoving) {
		inertia[0] = (event.clientX - eventPos[0]) / 100;
		inertia[1] = (eventPos[1] - event.clientY) / 100;
		eventPos[0] = event.clientX;
		eventPos[1] = event.clientY;
	}
})

addEventListener("touchmove", (event) => {
	if (isMoving) {
		inertia[0] = (event.screenX - eventPos[0]) / 1000;
		inertia[1] = (eventPos[1] - event.screenY) / 1000;
		eventPos[0] = event.screenX;
		eventPos[1] = event.screenY;
	}
})
...
addEventListener("touchstart", (event) => {
	if ( !isMoving ) {
		isMoving = true;
		eventPos = [event.screenX, event.screenY];
	}
})
...
addEventListener("mousedown", (event) => {
	if ( !isMoving ) {
		isMoving = true;
		eventPos = [event.clientX, event.clientY];
	}
})
